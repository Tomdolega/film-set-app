...
import { presentShootingDayAssignmentDetails } from "../../presenters/scheduling.presenter.js";

interface ListShootingDayAssignmentsControllerParams {
...
export function createListShootingDayAssignmentsController(
  params: ListShootingDayAssignmentsControllerParams,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      const shootingDayId =
        typeof request.params.shootingDayId === "string" ? request.params.shootingDayId : "";
      const assignments = await listShootingDayAssignments({
        shootingDayId,
      ...
      response.status(200).json(assignments.map(presentShootingDayAssignmentDetails));
    } catch (error) {
      next(error);
    }
  };
